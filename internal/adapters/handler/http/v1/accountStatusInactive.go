...
func (h *handler) AccountInactivate(w http.ResponseWriter, r *http.Request) {
	...
	req, err := request.NewAccountInactivate(r)
	if err != nil {
	...
	if account.Status == constant.ACCOUNT_STATUS_INACTIVE {
		resData := port.AccountInactivateClientResponse{
	...
	err = h.usecases.Account.AccountInactivate(ctx, req.GetAccountId(), req.GetUserId())
	if err != nil {
		res.SetStatus(http.StatusInternalServerError)
